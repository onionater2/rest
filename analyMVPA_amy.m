function analyMVPA_amy(roin, experiments,partition_names,condition_names_all,savedirectory,roinum, groups, group_loc,parametric,conORt, centering)
...
%Edited by AES Feb 2013 (small output organization changes. i.e. naming of results folders, added call to mvpaData2csv so that data get saved as csv too)
...
condition_names = condition_names_all{currentcond}; %%AES: this doesn't appear to accomplish anything??
    
namestring = ['MVPA_' num2str(currentcond) '_' roin];
filenameout=[namestring,'.csv']; %%AES: nothing is currently done with this? i think would be used if plotting accuracies
cd(savedirectory)
...
%roispath = directory;
roiindexes=[1];  %% this whole thing is irrelevant given how we wrap and call for each ROI (AES)
                ...
                %   data_part2=rex(char({datafilename{:,part2,ncondition}}),roi,'level','voxels','select_clusters',0,'selected_clusters',roinumber,'disregard_zeros',0); % subjects x voxels data
                    ...
                    for s = 1:length(experiments.data) %for all subjects 
                          ...
                          if groups == 1
                                ...
                                data_part1(ncondition).(thesubj) = rex(char({datafilename{s,part1,ncondition}}),roi,'level','voxels','select_clusters',0,'selected_clusters',roinumber,'disregard_zeros',0)'; % subjects x voxels data %% AES: this should be the vector of con values for the voxels in a given subjects roi (for condition ncondition)
...
%                     data_part2=rex(file2,roi,'level','voxels','select_clusters',0,'selected_clusters',roinumber,'disregard_zeros',0); % subjects x voxels data
            ...
            % gets rid of nans and centers (across conditions)
                ...
                for subj = 1:nsubjects
                        ...
                        for n = 1:length(Data_part1) %% AES: for each voxel, keep it only if ~nan? does this actuall remove voxel if it's nan for any subj?
...
%AES: this does something very wrong and weird. something off in implementation of centering: do not use until you figure out                        
%                          condmean1 = mean([Data_part1(1:length(Data_part1)).(names{subj})],2); %% gets mean across row (so separate mean for each voxel) 
                ...
                for ncondition1=1:numel(condition_names),
                    for ncondition2=1:numel(condition_names),
                        if numel(idxvoxels)>=minimumvoxels,
                           ...
                           clear r %% same variable used for centered and uncentered r might be problem?
                        else 
...
if centering == 0  %% AES: calculates centered and uncentered Rs regardless (see above). this just determines which is reported in the summary/tstats
...
%acc_pairwise (AES: if you want to do haxby style accuracy analysis)
...
%% don't do this for each discrimination mvpaData2csv(['MVPA_data_' roin '.mat'],['MVPA_data_' roin '.csv']);
