function saxelab_model_2012_oldshift(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% HELP
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% saxelab_model_2012_oldshift(varargin)
%
...
if length(subjs)>1
    for s = 1:length(subjs)
        if length(varargin)>4
            saxelab_model_2012(study,subjs{s},task,bolds{s},varargin{5:end});
        else
            saxelab_model_2012(study,subjs{s},task,bolds{s});
        end
    end
    return
end
...
if length(varargin) > 4
    ...
    for ovs = 1:2:length(othervars)
        ...
        if ~ischar(cov{1})
            warning(sprintf('Extra variable %i is not a string',(ovs+1)/2));
            dne = 1;
        end
        % validation step 2: ensure that cov{1} is a variable that can be 
        % changed
        if ~dne&&~any(strcmp(cov{1},user_config_vars))
            warning(sprintf('%s is not a user-configurable variable',cov{1}));
            dne = 1;
        end
        if ~dne&&~isnumeric(cov{2})
            warning(sprintf('%s is assigned a non-numeric value',cov{1}));
        end
        if ~dne
            def.(cov{1}) = cov{2};
        end
