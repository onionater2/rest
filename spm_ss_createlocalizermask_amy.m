function maskfilename=spm_ss_createlocalizermask_amy(SPM,Ic,Ec,overwrite,thr_type,thr)
% just a local copy of SPM_SS_CREATELOCALIZERMASK
% thresholds a given contrast (uncorrected p-value or FDR-corrected p-value thresholds are implemented).For each selected contrast this script will create a new volume named locT_####.img containing a binary map resulting from thresholding the selected contrast (spmT_###.img file) at the chosen level.

% thr_type= 'FDR', 'FWE', 'none'
% e.g. spm_ss_createlocalizermask_amy({},[],[],1,'FWE', .001)
...
posstr=1;
if isempty(SPM),
    str='Select SPM.mat(s) (containing relevant contrast)';
    disp(str);
