...
hdat = line(sx,y,'LineStyle','none','Marker','+', 'MarkerSize', 10);
%%AES added color coding for difference from normality
sigdifffromnormal=0;
if length(x)>4
sigdifffromnormal=lillietest(x);
end
if sigdifffromnormal
    color='g';
else
    color='r';
end
if m==1
    set(hdat,'MarkerEdgeColor', color);
    set([hrefends,hrefmid],'Color','b');
end
if nargout>0
...
xlabel('stats:probplot:Data');
ylabel('stats:probplot:Probability');
